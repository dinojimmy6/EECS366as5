...
void ComputeIntersection(Vertex S, Vertex E, int edge, Vertex *v) {
	float nx, ny, nz, p0x, p0y, p0z;
	switch (edge) {
	case 0: nx = 1; ny = 0; nz = 0; //plane of intersection
		    p0x = 1; p0y = 0; p0z = 0;
			break;
	case 1: nx = -1; ny = 0; nz = 0;
			p0x = -1; p0y = 0; p0z = 0;
			break;
	case 2: nx = 0; ny = 1; nz = 0;
		    p0x = 0; p0y = 1; p0z = 0;
		    break;
	case 3: nx = 0; ny = -1; nz = 0;
			p0x = 0; p0y = -1; p0z = 0;
			break;
	case 4: nx = 0; ny = 0; nz = 1;
			p0x = 0; p0y = 0; p0z = 1;
			break;
	case 5: nx = 0; ny = 0; nz = -1;
			p0x = 0; p0y = 0; p0z = -1;
			break;
	}
	float d = ((p0x - S.x) * nx + (p0y - S.y) * ny + (p0z - S.z) * nz) / ((E.x - S.x) * nx + (E.y - S.y) * ny + (E.z - S.z) * nz);
	float ix = d * (E.x - S.x) + S.x;
	float iy = d * (E.y - S.y) + S.y;
	float iz = d * (E.z - S.z) + S.z;
	float ih = d * (E.h - S.h) + S.h;
	v->x = ix; v->y = iy; v->z = iz; v->h = ih;
}
...
Vertex* ClipPolygon(int count, Vertex* input, int* out_count)
{
	...
	*out_count = count;
	...
	Vertex* outputList = new Vertex[20];
	int oIndex = 3; //initial list is of size 3
	for (int i = 0; i < oIndex; i++) {
		outputList[i].x = input[i].x; outputList[i].y = input[i].y; outputList[i].z = input[i].z; outputList[i].h = input[i].h;
	}
	for (int i = 0; i < 6; i++) {
		Vertex* inputList = new Vertex[oIndex];
		//Copy the outputList into the input list
		for (int j = 0; j < oIndex; j++) {
			inputList[j].x = outputList[j].x; inputList[j].y = outputList[j].y; inputList[j].z = outputList[j].z; inputList[j].h = outputList[j].h;
		}
		for (int j = 0; j < 20; j++) { //clear outputList
			outputList[j].x = 0; outputList[j].y = 0; outputList[j].z = 0; outputList[j].h = 0;
		}
		Vertex S = inputList[oIndex - 1]; //last vertex of the list
		int nIndex = 0;
		for (int j = 0; j < oIndex; j++) {
			Vertex E = inputList[j];
			if (inEdge(i, E)) {
				if (!inEdge(i, S)) {
					ComputeIntersection(S, E, i, outputList + nIndex);
					nIndex++;
				}
				outputList[nIndex].x = E.x; outputList[nIndex].y = E.y; outputList[nIndex].z = E.z; outputList[nIndex].h = E.h;
				nIndex++;
			}
			else if (inEdge(i, S)) {
				ComputeIntersection(S, E, i, outputList + nIndex);
